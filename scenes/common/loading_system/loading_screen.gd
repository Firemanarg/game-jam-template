...
func _process(delta: float) -> void:
	loading_bar.set_value(_progress * 100)
...
func _start_load() -> void:
	...
	for path: String in paths_to_load:
		Loading.load_async(
				path,
				_on_load_finished.bind(path, progress_index),
				_on_progress_updated.bind(progress_index))
		progress_index += 1
...
func _on_progress_updated(progress: float, index: int) -> void:
